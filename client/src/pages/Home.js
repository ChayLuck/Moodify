    ...
    <div className="bg-gray-900 min-h-screen text-white pb-32">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
            {movies.map((movie) => (
              <div key={movie.id} onClick={() => openMovieModal(movie.id)} className="bg-gray-800 rounded-xl overflow-hidden hover:shadow-yellow-500/30 hover:shadow-2xl transition duration-300 transform hover:-translate-y-2 group cursor-pointer">
                ...
                <div className="p-4">
                  <h3 className="font-bold truncate text-lg group-hover:text-yellow-400 transition">{movie.title}</h3>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="container mx-auto px-6 mt-20">
        ...
        {loading ? <p className="text-center text-gray-500">Loading...</p> : (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
            {songs.map((song) => (
              <div key={song.id} onClick={() => openTrackModal(song.id)} className="bg-gray-800 rounded-xl overflow-hidden hover:shadow-blue-500/30 hover:shadow-2xl transition duration-300 transform hover:-translate-y-2 group relative cursor-pointer">
                <div className="relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300">
                        <span className="bg-green-500 text-black rounded-full w-14 h-14 flex items-center justify-center text-2xl shadow-lg">🔍</span>
                    </div>
                </div>
                <div className="p-4">
                    ...
                    <>
                        ...
                        <div className="w-full md:w-2/3 p-8 flex flex-col">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                ...
                                <div><h4 className="text-white font-bold mb-2 border-b border-gray-700 pb-1">Cast</h4>
                                    <div className="flex flex-col gap-2">{selectedMovie.cast?.map(actor => (<div key={actor.name} className="flex items-center gap-3"><img src={actor.photo || "https://via.placeholder.com/50"} alt={actor.name} className="w-8 h-8 rounded-full object-cover"/><div><p className="text-sm text-white">{actor.name}</p></div></div>))}</div>
                                </div>
                            </div>
                            {/* BUTONLAR BURADA */}
                            <div className="mt-auto pt-4 border-t border-gray-700 flex gap-4">
                                ...
                                <button onClick={() => alert("Coming soon!")} className="flex-1 bg-yellow-600 hover:bg-yellow-500 text-black py-3 rounded-lg font-bold">❤️ Favorite</button>
                            </div>
                        </div>
                    </>
                )}
      ...
      {/* --- MUSIC MODAL (BİLGİLER VE PLAYER DÜZELTİLDİ) --- */}
      {selectedTrack && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in" onClick={closeModal}>
                    ...
                    <>
                        ...
                        <div className="w-full md:w-1/2 p-8 flex flex-col justify-center">
                            ...
                            <div className="flex gap-4 mt-auto">
                                ...
                                <button onClick={() => initiateFavorite(selectedTrack)} className="flex-1 bg-gray-800 hover:bg-gray-700 text-white py-3 rounded-lg font-bold border border-gray-600">❤️ Favorite</button>
                            </div>
                        </div>
                    </>
                )}
        ...
        <div className="fixed bottom-0 left-0 w-full bg-black/90 border-t border-green-900 p-4 backdrop-blur-lg z-[70] animate-slide-up shadow-2xl">
            <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
                <div className="flex-1">
                    {/* 👇 BURASI DÜZELTİLDİ: https://open.spotify.com/embed/track/... */}
                    <iframe 
